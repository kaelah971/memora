...
export function createDiscordListenerStorage(): DiscordListenerStorage {
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error("Authentication is required before database workspace access is enabled in production.");
  }
  if (config.missingPublic.length > 0 || !config.serviceRoleConfigured) {
    throw new Error("Configure Supabase public variables and SUPABASE_SERVICE_ROLE_KEY for local data access.");
  }
  if (!config.developmentAccessEnabled) {
    throw new Error("Set MEMORA_DEV_DB_ACCESS=service_role to explicitly enable local database reads.");
  }
  ...
  try {
    ...
    throw new Error(error instanceof Error ? error.message : "Supabase configuration could not be loaded.");
  }
}
